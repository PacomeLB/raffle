...
contract RaffleAdminPayTaxVrf is AbstractRaffleOptimized, ERC721URIStorage
{
    ...
    error TokenURIlengthMustMatchMaxTickets(uint8 tokenURIlength, uint8 maxTickets);

    uint8 private immutable taxRate;
    uint256 public vrfRequestId;
    bool private isVrfRequested = false;

    event RaffleAdminTaxDeployed(string message,
        address _admin,
        address _benfit,
        address _tax,
        uint8 _taxRate,
    ...
    event RaffleAdminTaxPaid(string message, uint256 taxAmount, address taxSentTo);
    ...
    {
        require(_maxTickets*_ticketPrice > _prize, "Prize must superior to the sum of all tickets");
        ...
        emit RaffleAdminTaxDeployed("Raffle admin has been deployed", admin, benefit, tax, taxRate, _name);
    }

    /**
     * Launch the raffle to get a winner
     */
    function launchRaffle() public override onlyAdmin("Only admin can launch raffle") nonReentrant()
    {
        ...
        require(isVrfRequested == false, "Raffle has a pending VRF request pending, wait for the oracle to reply");
     ...
     */
    function withdrawBenefit(address payable /*_unused*/) public override onlyAdmin("Only admin can withdraw benefits") nonReentrant()
    {
        require(isRaffleFinished == true, "Raffle is not yet finished, contract balance will be available once a winner has been drawn"); 
        ...
        emit WithdrawBallance("Benefit of the Raffle has been withdraw to", benefitToSend, benefit);
        _resetContract();
    }
     ...
     */
    function resetContract() external override onlyAdmin("Only admin can reset the contract") nonReentrant() 
    {
        withdrawContract();
        isRaffleFinished = false;
        resetNTFTickets();
     ...
     */
    function setRandomNumber(uint256 _requestId, uint256[] calldata _randomNumbers) external onlyRaffleVRF("Only VRF contract can set the random number") nonReentrant
    {
        require(_requestId == vrfRequestId, "Random  number request does not match current request!");
